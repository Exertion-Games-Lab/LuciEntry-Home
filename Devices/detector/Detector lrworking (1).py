...
def detection(commandParameters=[], board_name = "SYNTHETIC"):
    ...
    # while myArgs.isInterrupt==False:
        ...
        #         yasa_data_eog = yasa_data_eog[8950:]
            ...
            time.sleep(0.2)
            if board.get_board_data_count() < 1700:
                continue
            timeoutCnt+=1
            ...
            #DataFilter.perform_notch(eog_data[eog_channel_left], 150, 1)
            DataFilter.perform_bandpass(eog_data[eog_channel_left], sampling_rate, 0.3, 6, 4, FilterTypes.BUTTERWORTH.value, 0)
            DataFilter.perform_bandpass(eog_data[eog_channel_right], sampling_rate, 0.3, 6, 4, FilterTypes.BUTTERWORTH.value, 0)
            ...
            DataFilter.detrend(eog_data_yasa[eog_channel_left], DetrendOperations.LINEAR.value)
            DataFilter.detrend(eog_data_yasa[eog_channel_right], DetrendOperations.LINEAR.value)
            DataFilter.perform_bandpass(eog_data_yasa[eog_channel_left], sampling_rate, 0.5, 6, 4, FilterTypes.BUTTERWORTH.value, 0)
            DataFilter.perform_bandpass(eog_data_yasa[eog_channel_right], sampling_rate, 0.5, 6, 4, FilterTypes.BUTTERWORTH.value, 0)
            DataFilter.perform_rolling_filter(eog_data_yasa[eog_channel_left], 5, AggOperations.MEDIAN.value)
            DataFilter.perform_rolling_filter(eog_data_yasa[eog_channel_right], 5, AggOperations.MEDIAN.value)

            yasa_eog_comb = np.vstack((eog_data_yasa[eog_channel_left],eog_data_yasa[eog_channel_right]))
            yasa_eog_comb = yasa_eog_comb.reshape(2,-1)
            try:
                rem = yasa.rem_detect(yasa_eog_comb[0,:],yasa_eog_comb[1,:],sampling_rate)
            ...
            if L_count == 0 and eog_class == "right":
               N_count += 1
            if N_count == 10: #error threshold so if there are 3 missed turns it resets
               L_count = 0
